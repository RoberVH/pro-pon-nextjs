...
import { readUserFile } from "../../utils/readUserFile";
...
const DownloadFileForm = ({
  ...
  const toggleAllEntries = () => {
    setDownloadableFiles((prevFiles) =>
      prevFiles.map((prevFile) => {
        return { ...prevFile, selected: !selectAll };
      })
    );
  ...
  const handleDownloadAllSelectedFiles = async () => {
    ...
    const privateFiles = selectedFiles.filter((file) =>
      privateFileTypes.includes(parseInt(file.docType.toString()))
    );
    ...
    for (const file of selectedFiles) {
      try {
        ...
        setProcessedFiles((prevFiles) =>
          prevFiles.map((elem) =>
            elem.idx === file.idx ? { ...elem, status: "processing" } : elem
          )
        );
        ...
        setProcessedFiles((prevFiles) =>
          prevFiles.map((elem) =>
            elem.idx === file.idx ? { ...elem, status: "success" } : elem
          )
        );
      } catch (error) {
        setProcessedFiles((prevFiles) =>
          prevFiles.map((elem) => {
            if (elem.idx === file.idx)
              return { ...elem, status: "failure", msg: t(error.msg) };
            else return elem;
          })
        );
      }
  ...
  const saveBlobAsFile = async (option, filePath, writableBlob, filename) => {
    let blob;
    if (option === "filePath") {
      // is a public file that needs to be download from Arweave
      try {
        const url = filePath;
      ...
      return (
        <div className="mt-4 py-1 bg-white mb-2 shadow-lg ">
          {processedFiles.map((file, index) => (
            <div
              key={index}
              className={`text-sm grid grid-cols-[40%,55%,5%] text-stone-700 border-t border-orange-500`}
            >
              <div
                className={`${statusClass(
                  file?.status
                )} border-r border-l border-orange-500 
              ${
              ...
              <div
                className={`${statusClass(
                  file?.status
                )} text-center border-r border-l border-orange-500 justify-center text-xl font-bold pb-1
              ${
                index === processedFiles.length - 1
                  ? "border-b border-orange-500"
                  : ""
  ...
  // copy the passed text to clickboard and briefly turn on flag to show message copied to clipboard
