  ...
  <div className="m-auto py-2 max-w-[90%] ">
    ...
    <div  className=" max-h-[44rem] overflow-y-auto">
      ...
      className="table-fixed border-collapse font-khula font-bold text-stone-700 h-full
        ...
        <thead className="">
          <tr className="  bg-orange-100  text-left text-xl text-stone-600 font-extrabold">
            <th className="p-4 w-1/3 border-2 border-stone-300">
                <strong>{t('document_name')}</strong>
                </th>
            <th className="p-4 border-l-4 border-coal-400"><string>{t('document_hash')}</string></th>
          </tr>
        </thead>
        <tbody className="">
          {downloadableFiles.map(file => (
            <tr key={file.idx} className="even:bg-slate-200 odd:bg-slate-300 ">
              ...
              <td className=" truncate  border-2 p-2 even:border-2 even:border-slate-300">{file.documentHash}</td>
            </tr>
          ))}
        </tbody>
      </table>
