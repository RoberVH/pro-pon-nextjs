...
function UploadRFPForm({ t, setNewFiles, rfpId, rfpIndex, docType, owner }) {
  ...
  const ShowSummaryUploads =() => 
    <>
    {console.log('sendingBlockchain en html',sendingBlockchain)}
    <p> <strong>{`${successFiles.length}`} </strong> {t('files_uploaded')} </p>
    ...
    {link &&  
      ...
      <div className="flex my-4 mx-auto ">
        <button 
            onClick={recordRFPMetadatatoContract} 
            className="main-btn ">
            {!sendingBlockchain ? 
                <span>  {t('accept')} </span> : 
...
// alert parent component homeRFP there are new RFP files it must update for all child components
  useEffect(() => {
    if (blockchainsuccess) setNewFiles(true)
 ...
 */
  const recordRFPMetadatatoContract = async () => {
    ...
    console.log('Entrando a recordRFPMetadatato con bandera:',sendingBlockchain)
    setsendingBlockchain(true)
    console.log('Ahora bandera:',sendingBlockchain)
    // let's upload to Contrat
    // create params arrays for createRFP contract method
    const nameArray = uploadingSet.filter(doc => (doc.status==='success'))
      .map(filtered => ( filtered.name))
    const hashArray = uploadingSet.filter(doc => (doc.status==='success'))
      .map(filtered => ( filtered.hash))
    const fileIdArray = uploadingSet.filter(doc => (doc.status==='success'))
      .map(filtered => ( filtered.fileId))  
    try {  // write metadata files to contract  
  ...
  const uploadArweave = useCallback(
    async (pickedFiles) => {
      ...
      const processFilePromisesArray = pickedFiles.map((file, indx) => 
          uploadBlockchainFile(setuploadingSet, file, indx,owner,rfpId, RmteBundlr, ArweavefileTypes.requestFile, rfpIndex)
        )
