...
const ShowResults = ({ t, rfpIndex }) => {
  ...
  useEffect(() => {
    async function getWinnersData() {
      ...
      try {
        let result;
        if (window.ethereum) result = await getContractRFP(rfpIndex);
        else result = await getContractRFPFromServer(rfpIndex);
        if (!result.status) {
        ...
        if (window.ethereum)
          ...
          if (result.status) winnersFromContract = result;
        ...
        if (winnersFromContract.status) {
          ...
          const newWinners = winners.filter((elem) =>
  ...
  const processWinner = (address) => {
    if (address === NullAddress) return t("deserted_items");
    else {
      const companyData = winnersData.filter(
        (winner) => winner.address.toLowerCase() === address.toLowerCase()
      );
      if (companyData)
        return `${companyData[0].companyId} - ${companyData[0].companyname}`;
      else return `${t("cannot_retrieve_company")} - ${address}`;
    }
  };
      ...
      return (
        <>
          {rfpRecord.items.map((item, index) => (
            <tr
              key={index}
              className="text-components font-khuna text-stone-900 text-bold"
            >
  ...
  if (rfpRecord.canceled)
    return <TitleDeclare msg={t("cancel_notice")} redflag={true} />;

  // if not yet awarded return notice
  if (winnersAddress?.length === 0)
    return <TitleDeclare msg={t("waiting_results")} />;
              ...
              <table className="mb-4 table-fixed font-work-sans mx-auto ">
                <thead>
                  <tr className="">
                    ...
                    <th className="w-2/5 px-4 py-2 border border-orange-300 text-components">
                      {t("winner")}
                    </th>
                  </tr>
                </thead>
