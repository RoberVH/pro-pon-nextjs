...
const DeclareResults = ({
  ...
  // list of  companies do not have email field value in profile
  const [compWithoutEmail, setCompWithoutEmail] = useState([]);
  // flag to alert some companies weren't informed of RFP declaration
  ...
  // notify participants that RFP has been declared
  const onSuccess = () => {
   ...
   * Sent notifications to companies letting them know the RPF is been declared
  ...
  const automaticEmailNotification = async (typeOfNotification) => {
    ...
    for (const company of companies) {
      if (company.email || (company.email && company.email.trim() !== "")) {
        ...
        CompWithoutEmail.push(company.companyname);
      }
    }
    const params = {
    ...
    if (recipientList.length) {
      ...
      toast.success(
        ` 0 ${t("notif_sent_ok", { ns: "common" }, toastStyleSuccess)}`
      );
    }
  };

  /*  Hooks ************************************************************************************** */
  const { write, postedHash, block, blockchainsuccess } = useDeclareResults(
    onError,
    onSuccess,
    isCancelled,
    setProTxBlockchain
  ...
  useEffect(() => {
    async function getRFP() {
      ...
      setRFPRecord(RFP);
      setWinners(
        Array(RFP.items.length != 0 ? RFP.items.length : 1).fill("not_choose")
      );
      setDeclareingItems(
        RFP.items.length != 0 ? RFP.items : [`${RFP.name} - ${RFP.description}`]
      );
  ...
  useEffect(() => {
    if (companies && companies.length !== 0) {
      for (let i = 0; i < companies.length; i++) {
        if (
          Object.keys(companies[i]).length === 0 ||
          "status" in companies[i]
        ) {
  ...
  const handleDeclareRFPCanceled = () => {
    ...
    const Tx = {
      type: "cancelRFP",
      date: today,
      params: [rfpRecord.rfpIndex, rfpRecord.companyId, [], true],
    };
    setDroppedTx(Tx);
  ...
  const handleDeclareRFPWinners = async () => {
    ...
    const Tx = {
      type: "declareRFP",
      date: today,
      params: [rfpRecord.rfpIndex, rfpRecord.companyId, winners, false],
    };
    setDroppedTx(Tx);
  ...
  const CancelRFPComponent = () => {
    const handleToggleCollapsibleSection = () => {
        ...
        <div className="border border-orange-300  rounded-md shadow p-4">
          <div className="flex justify-between items-center">
            ...
            <div
              ...
              onClick={handleToggleCollapsibleSection}
            >
              {isCollapsed ? (
                <Image alt="V" src="/chevrondown2.svg" width={22} height={22} />
              ) : (
  ...
  if (!doneLookingBidders || gettingRFP)
    return (
      <div className="mt-24">
        <Spinner />
      </div>
  ...
  return (
    <div className="mx-auto mt-8  p-4 w-[90%] border border-orange-100  shadow-md shadow-orange-100">
      ...
      {isCollapsed && (
        <>
          <WinnersTable items={declaringItems} competitors={companies} />
