...
const PendingTxs = ({
  ...
  useEffect(() => {
    const getTxStatuses = async () => {
      ...
      if (statuses) setTxStatuses(statuses);
      setIsLoading(false);
    };
    getTxStatuses();
  ...
  const txButtonsActions = (status, type, txId, Tx) => {
    if (status === "pending" && type === "filesuploadm")
      //debugin, should be (status==='pending' && type==='filesuploadm')
      return (
        <div>
          <button
  ...
  return (
    <div>
      <div className=" flex justify-center items-center  mb-4">
        <h2 className="text-titles text-stone-500 mt-8 mx-auto">
          {t("mypending_transactions")}
        </h2>
        <button
          disabled={actionButtonClicked}
      ...
      {isLoading ? (
        ...
        <div>
          <div
            key="0001_HEADER"
            className="text-components flex items-center justify-center bg-sky-600 rounded-lg py-2 mb-2 font-work-sans  
          ...
          {Boolean(txStatuses.length) &&
            txStatuses
              .sort((a, b) => a.date - b.date)
              .map((tx) => (
                <div
                  key={tx._id}
                  className="text-titles-div flex items-center justify-center bg-orange-100 rounded-lg py-2 mb-2 font-work-sans text-stone-700 font-semibold shadow"
                >
                  ...
                  <div
                    id="3rdcol"
                    className="w-[15%] ml-4 text-orange-500 truncate cursor-pointer hover:text-orange-700"
                    onClick={() =>
                      window.open(
                        `${process.env.NEXT_PUBLIC_LINK_EXPLORER}/tx/${tx.txHash}`,
                        "_blank"
                      )
                    }
                  ...
                  <div
                    id="4th"
                    className="w-[30%] flex items-center justify-between "
                  >
                    {tx.status === "pending" && (
                      <div className="text-red-700 font-bold mr-2">
                        {" "}
                        {t("pending", { ns: "rfps" })}{" "}
                      </div>
                    )}
                    {tx.status === 1 && (
                      <div
                        className="font-bold text-green-700  mr-2 cursor-pointer "
                        onClick={() =>
                          window.open(
                            `${process.env.NEXT_PUBLIC_LINK_EXPLORER}/block/${tx.blockNumber}`,
                            "_blank"
                          )
                        }
