/**
 * parseWeb3Error
 *      Analyze object error and return a string in the language given by useTranslation object in parameter t describing the error
 */
import { errorSmartContract } from "./constants"

export const parseWeb3Error = (t, error) => {
  console.log("parseweb3 recibio error:", error)
  ...
  if (typeof error.reason !== "undefined") {
    ...
    if (error && error.data && error.data.message) {
      ...
      if (customError.includes("err: insufficient funds for gas"))
        customError = t("insufficient_funds", { ns: "gralerrors" })
      ...
      if (
        ...
        customError = t("user_rejection", { ns: "gralerrors" })
      } else if (error && error.message === "requestaccounts_pending") {
      ...
      } else if (error && error.message.toUpperCase().includes("USER REJECTED"))
        customError = t("user_rejection", { ns: "gralerrors" })
      else if (error && error.message.includes("bad_method"))
